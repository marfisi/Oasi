...
@NamedQueries(
	{@NamedQuery(
		name = "AsForni00f.findAll",
		query = "SELECT a FROM Forni00f a order by a.focfor"
	), @NamedQuery(
		name = "AsForni00f.findByFocfor",
		query = "SELECT a FROM Forni00f a WHERE a.focfor = :focfor"
	), @NamedQuery(
		name = "AsForni00f.findAggiornatiDopo",
		query = "SELECT a FROM Forni00f a WHERE a.fouins > :fouins or (a.fouins = :fouins and a.fohins > :fohins) order by a.focfor"
	)
	}
)
