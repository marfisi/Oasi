...
/**
* The persistent class for the OA_CarichiTestate database table.
* 
*/
@Entity(
	name = "OA_CarichiTestate"
)
@NamedQueries(
	{@NamedQuery(
		name = "MsvOA_CarichiTestate.findAll",
		query = "SELECT o FROM OA_CarichiTestate o WHERE substring(o.tipoOperazione, 1, 1) != '*' and o.tipoOperazione != 'DEL' order by o.dataReg, o.idUnivocoTes"
	), @NamedQuery(
		name = "MsvOA_CarichiTestate.findByIdUnivocoTes",
		query = "SELECT o FROM OA_CarichiTestate o WHERE o.idUnivocoTes = :idUnivocoTes"
	), @NamedQuery(
		name = "MsvOA_CarichiTestate.findByIdPntNReg",
		query = "SELECT o FROM OA_CarichiTestate o WHERE o.idPntNReg = :idPntNReg"
	)
	}
)
public class MsvOA_CarichiTestate implements Serializable{
	...
	public boolean equals(Object obj){
		if(obj instanceof MsvOA_CarichiTestate){
			if(this.idUnivocoTes == ((MsvOA_CarichiTestate)obj).idUnivocoTes){
				return true;
			}else{
				return false;
			}
